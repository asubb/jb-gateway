...
def parse_arguments():
    ...
    parser.add_argument("command", nargs="*",
                        help="Command to execute (if not provided, starts a regular container shell)")
    return parser.parse_args()

def execute_host_command(command, working_dir=None):
    """
    ...
    if working_dir is None:
        raise RuntimeError("Working directory must be specified to execute commands on host")
...
def interactive_host_shell():
    ...
    username = "root" # won't change at the moment
    ...
    while True:
        try:
            ...
            if command.strip().startswith("cd "):
                ...
                cd_result = execute_host_command(f"cd {new_dir}", working_dir)
                if cd_result == 0:
                    working_dir = new_dir
                ...
                execute_host_command(command, working_dir)
